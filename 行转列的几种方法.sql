...
/*
crosstab函数为具有相同row_name值的每个连续输入行组生成一个输出行。使用value这些行中的字段从左到右填充输出列。如果组中的行少于输出value列，则额外的输出列将填充空值; 如果有更多行，则跳过额外的输入行，实际上，SQL查询应始终指定ORDER BY 1,2(这里的1，2是相对位置，select选中的第几个)以确保输入行的顺序正确，即具有相同值的值row_name汇集在一起​​并在行中正确排序。类似pivot(name,info,value)
*/
SELECT *
FROM crosstab('select name, info, value from old_table order by 1,2')
